...
def build_parser():
    parser = argparse.ArgumentParser(description="MetaWear acceleration sample client built on NexusBLESdk.")
    ...
    parser.add_argument(
        "--sampling-rate-hz",
        type=int,
        choices=[METAWEAR_ACCEL_ODR_HZ],
        default=METAWEAR_ACCEL_ODR_HZ,
        help="MetaWear first pass supports acceleration only at 100 Hz.",
    )
    ...
    parser.add_argument(
        "--write-to-file",
        action="store_true",
        help="Write parsed MetaWear acceleration stream values to output-files/ in the current working directory.",
    )
    parser.add_argument(
        "--dump-raw-file",
        help="Optional JSONL file for raw MetaWear frame capture.",
    )
...
def run(args) -> int:
    with open_gateway_serial(args.port) as ser:
        client = GatewayClient(ser, client_name="metawear_stream_client")
        metawear = MetaWearClient(client)
        ...
        if args.dump_raw_file:
            ...
            metawear.set_raw_dump_file(raw_dump_file)

        if args.write_to_file:
            parsed_output_path = build_output_path("metawear_accel_stream", "csv")
            ...
            metawear.set_parsed_row_writer(parsed_row_writer)
        ...
        client.phase = "scan"
        selected = metawear.discover(args.sensor_count, args.scan_timeout_ms)
        print(f"Selected addresses: {selected}")

        client.phase = "connect"
        connections = metawear.connect(selected, timeout_s=args.connect_timeout_s)
        ...
        try:
            ...
            print(f"Starting MetaWear acceleration stream. Total stream budget: {args.stream_seconds}s.")

            started_at = metawear.start_streams(
                write_timeout_s=args.write_timeout_s,
                without_response=args.without_response,
            )
            ...
            metawear.disconnect_all(timeout_s=args.disconnect_timeout_s)
